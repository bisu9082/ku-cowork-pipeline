...
################################################################
# [PROVENANCE AUDIT] — 제출 전 출처·메타데이터 감사
# 연결 도구: github.com/guillaumemeyer/watermarks-remover (audit-only)
################################################################

## 허용 목적
- C2PA, Content Credentials, AI metadata, invisible Unicode carrier, stylometry signal을 검사한다.
- 결과는 AI-use disclosure, 출처표기, 저널 정책 준수, 민감 로컬 메타데이터 확인에만 사용한다.
- 표준 정책 파일: `pipeline/metaclaw/provenance_audit_policy.md`
- 교차 도구 registry: `pipeline/metaclaw/provenance_tools.json`
- 1차 실행 래퍼: `scripts/run_provenance_audit.py`
- 교차 실행 래퍼: `scripts/run_cross_provenance_audit.py`

## 금지 목적
❌ AI provenance/C2PA/저작권/출처표시 제거·은폐·우회 금지
❌ detector 회피 목적 rewrite 금지
❌ 자동 clean/removal 엔트리포인트 호출 금지
❌ noai-watermark/visible-remover 계열을 제출물 정리 자동단계로 실행 금지 — 수동 검토·disclosure 판단 참고만 허용

## Step 7/8 Gate
Step 7 인용 검증 전 또는 제출 패키지 생성 후:
`scripts/run_cross_provenance_audit.py [제출폴더] --watermarks-dir watermarks-remover --out workspace/Step7/cross_provenance_audit.json`

완료 보고에 반드시 포함:
`PROV-CROSS: [pass/review-needed] | reports: [cross json, primary json] | disclosure: [none/needed/included]`
